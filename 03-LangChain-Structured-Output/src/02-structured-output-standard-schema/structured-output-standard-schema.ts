...
const RawReviewSchema = v.object({
    key_themes: v.pipe(v.array(v.string()), v.description("Write down all the key themes discussed in the review in an array")),
    summary: v.pipe(v.string(), v.description("A brief summary of the review")),
    sentiment: v.pipe(v.picklist(["pos", "neg"]), v.description("Return sentiment of the review either negative, positive or neutral")),
    pros: v.optional(v.pipe(v.array(v.string()), v.description("Write down all the pros inside a list"))),
    cons: v.optional(v.pipe(v.array(v.string()), v.description("Write down all the cons inside a list"))),
    name: v.optional(v.pipe(v.string(), v.description("Write the name of the reviewer"))),
});

type ReviewType = v.InferOutput<typeof RawReviewSchema>;

const ReviewSchema = toStandardJsonSchema(RawReviewSchema);
